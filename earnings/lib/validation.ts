import {baseCandidates} from './catalog';
import type {Candidate,Person,Pick} from './types';
...
export function validatePicks(value:unknown,candidates:Candidate[]=baseCandidates):Pick[]{
 ...
 return value.map(p=>{
  ...
  if(!c)throw new ValidationError('Choose a stock from the watchlist or add it first.');
...
export function validateSave(value:unknown,candidates:Candidate[]=baseCandidates){
 ...
 return {person,version:Number(v.version),picks:validatePicks(v.picks,candidates)};
}
